...
def convergence_radius(e_list, no_of_shanks):
    ratio = []
    n = len(e_list)
    for i in range(n-1):
    ...
    for i in range(no_of_shanks-1):
        shanks = shanks_transformation(shanks)
        plt.plot(shanks)
    plt.plot(ratio,label = 'Initial series')
    plt.xlabel("k")
    plt.ylabel("E(k+1)/E(k)")
    plt.title("Determination of radius of convergence for excited H2 at bond length = 1.5695 Angstrom")
    plt.legend(loc='best')
    plt.grid()
    plt.show()
...
def uhf_h_tot(fock_alpha, fock_beta, hcore_alpha, hcore_beta, mo_integrals):
    return 0

mol = gto.M(
    atom = 'H 0 0 0; H 0 0 1.5',  # in Angstrom
    basis = '631g',
    #spin = 1,
)
